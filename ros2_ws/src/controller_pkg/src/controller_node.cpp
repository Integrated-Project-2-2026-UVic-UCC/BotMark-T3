...
ControllerNode::ControllerNode() : Node("controller_node")
{
  ...
  // 2. Suscriptores de Estado y Seguridad (LiDAR, Pausa, Stop)
...
{
  // Si el robot está en STOP, rechazamos cualquier nueva misión
  std::lock_guard<std::mutex> lock(data_mutex_);
  if (is_stopped_) {
    RCLCPP_WARN(this->get_logger(), "Robot en estado STOP. Orden rechazada.");
...
void ControllerNode::execute(const std::shared_ptr<GoalHandleNavigate> goal_handle)
{
  ...
  while (rclcpp::ok()) {
    ...
    double rx, ry, ryaw;
    bool is_blocked, paused, stopped;
    {
      std::lock_guard<std::mutex> lock(data_mutex_);
      rx = current_x_;
      ry = current_y_;
    ...
    // --- NUEVA LÓGICA DE STOP ---
    if (stopped) {
      RCLCPP_ERROR(this->get_logger(), "¡ESTADO STOP ACTIVADO! Abortando misión actual.");
      publish_stop();
      goal_handle->abort(result); // Cancela la misión informando al Mission Manager
      return; // Salimos del bucle de ejecución
    }

    // --- LÓGICA COMBINADA DE OBSTÁCULO Y PAUSA ---
    if (is_blocked || paused) {
      if (is_blocked) {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "¡OBSTÁCULO! Esperando...");
      } else {
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "PAUSA ACTIVADA. Esperando reanudación...");
      }
      publish_stop();
      loop_rate.sleep();
      continue; // Saltamos el movimiento pero NO abortamos la misión
    }

    // Matemáticas de Control Cinemático
